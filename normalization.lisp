...
(defmethod ccons ((atom atom) (environment environment) (rail rail) pair)
  (cond ((eql (unwrap atom) 'simple) 
         (make-instance 'closure :name 'anonymous :lexical-environment environment :argument-pattern rail :body pair))
        ((eql (unwrap atom) 'reflect) 
         (make-instance 'reflective-closure :name 'anonymous :lexical-environment environment :argument-pattern rail :body pair))
        (t 
         (error (format nil "ccons: unknown procedure type: ~s" (unwrap atom))))))
...
(defmethod add-binding ((environment environment) (atom atom) (handle handle))
  (make-instance 'environment :bindings (cons (make-mapping atom handle) (bindings environment))))
...
(defmethod bind ((environment environment) (argument-pattern rail) (arguments rail))
  (cond ((empty-p argument-pattern) environment)
        ((= (length argument-pattern) (length arguments))
         (bind (add-binding environment (first argument-pattern) (first arguments)) (rest argument-pattern) (rest arguments)))
        (t
         (error (format nil "bind: Function called with the wrong number of arguments.")))))

(defmethod binding ((atom atom) (environment environment))
  (let ((mapping (find (unwrap atom) (bindings environment) :key (lambda (atom+value) (unwrap (car atom+value)))))) 
    (if (null mapping)
...
(defun normalize (internal-structure environment continuation)
  (cond ((normal-p internal-structure) (funcall continuation internal-structure))
        ((atom-p internal-structure) (funcall continuation (binding internal-structure environment)))
        ((rail-p internal-structure) (normalize-rail internal-structure environment continuation))
        ((pair-p internal-structure) (reduce (pcar internal-structure) (pcdr internal-structure) environment continuation))
        (t
         (error (format nil "normalize: Error trying to normalize non internal structure ~s" internal-structure)))))
...
(defun reduce (procedure arguments environment continuation)
  (normalize procedure 
             environment
             (lambda (procedure!)
               (cond ((reflective-p procedure!) (reduce-reflective procedure! arguments environment continuation))
                     ((abnormal-p procedure!) (reduce-abnormal procedure! arguments environment continuation))
                     (t 
                      (normalize arguments 
                                 environment
                                 (lambda (arguments!)
                                   (if (primitive-p procedure!)
                                      ; (if  (cl-closure-p (unwrap procedure!))
                                       ;      (apply (unwrap procedure!) (cl-list (unwrap arguments!)))
                                         (funcall continuation (wrap (apply (unwrap procedure!) (cl-list (unwrap arguments!)))));)
                                     (normalize (body procedure!)
                                                (bind (lexical-environment procedure!) (argument-pattern procedure!) arguments!)
                                                continuation)))))))))
  ...
  (let ((non-reflective-closure (de-reflect procedure!)))
    (normalize (body non-reflective-closure)
               (bind (lexical-environment non-reflective-closure)
                     ...
                     (wrap (make-instance 'wrapped-cl-list :cl-list (list environment continuation arguments))))
...
; set is actually a dynamically scoped function
...
(defun reduce-set (arguments environment continuation)
  ...
  (let ((atom (first arguments)) ;; do not normalize the symbol
    ...
    (normalize expression environment 
               (lambda (expression!)
                 (set-binding environment atom expression!)
  ...
  (let ((argument-pattern (first arguments))
    ...
    (funcall continuation
             (make-instance closure-class-name
                            :name 'anonymous
                            :body body 
                            :argument-pattern (wrap (make-instance 'wrapped-cl-list :cl-list (unwrap argument-pattern))) ;; has to be a rail!, make sure args wrapped changed
...
(defun reduce-apply (arguments environment continuation) ;; apply takes a symbol and a rail
  (normalize arguments 
             environment 
             (lambda (arguments!)
               (reduce (first arguments!) (unwrap (first (rest arguments!))) environment continuation))))

(defun reduce-apply-abnormal (arguments environment continuation) ;; apply-abnormal takes a symbol and a rail and an evironment
  (normalize arguments 
             environment
             (lambda (arguments!)
               (reduce (first arguments!) (unwrap (first (rest (rest arguments!)))) (unwrap (first (rest arguments!))) continuation))))
