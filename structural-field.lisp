...
;; Abnormal closures are primitive closures who's arguments are not all normalized when the closure is normalized
...
;; Helper class, for representing rail in CL
...
(defmethod cl-bool (smth)
  (error (format nil "cl-bool: ~s is not either *cl-true* or cl-false" smth))) 
...
(defmethod unwrap ((abnormal-closure abnormal-closure))
  (let ((cl-closure (make-instance 'cl-closure :closure abnormal-closure)))
    (set-funcallable-instance-function
     cl-closure
     (lambda (&rest args)
       (declare (ignore args))
       (error "Don't call abnormal closures within Common Lisp code.")))
    cl-closure))
...
(defmethod unwrap ((closure closure))
  ...
  (let ((cl-closure (make-instance 'cl-closure :closure closure)))
    (set-funcallable-instance-function
     ...
     (lambda (args)	
       (reduce closure
               (make-instance 'rail :cl-value (make-instance 'wrapped-cl-list :cl-list (list args)))
               *global* 
