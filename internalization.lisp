...
;; Rails
(set-macro-character #\] #'(lambda (stream char) (declare (ignore char)) (read stream t nil t) nil))
...
(defmethod identify-cl-boolean (smth)
  (error (format nil "Error while parsing ~s, $ is reserved syntax for booleans." smth)))
...
;; Handle (cf ')
(set-macro-character #\? #'(lambda (stream char) (declare (ignore char)) (internalize (read stream t nil t))))
...
(defmethod internalize ((wrapped-cl-list wrapped-cl-list))
  (make-instance 'rail :cl-value (make-instance 'wrapped-cl-list :cl-list (cl-list wrapped-cl-list)))) ;changed to wrap all vals
